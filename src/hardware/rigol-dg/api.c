...
static struct sr_dev_inst *probe_device(struct sr_scpi_dev_inst *scpi)
{
	...
	for (i = 0; i < ARRAY_SIZE(device_models); i++) {
		if (g_ascii_strcasecmp(hw_info->manufacturer, device_models[i].vendor))
			continue;
		if (g_ascii_strcasecmp(hw_info->model, device_models[i].model))
			continue;
	...
	devc->cmdset = cmdset;
        devc->device = device;
	devc->ch_status = g_malloc0(sizeof(struct channel_status) * (device->num_channels + 1));
		...
		case SR_CONF_PATTERN_MODE:
			if ((ret = rigol_dg_get_channel_state(sdi, cg)) == SR_OK) {
				*data = g_variant_new_string(
					 waveform_type_to_string(ch_status->wf));
			}
			break;
	...
	if (!cg) {
		...
		case SR_CONF_PATTERN_MODE:
			...
			for (i = 0; i < ch_spec->num_waveforms; i++) {
				mode_name = waveform_type_to_string(ch_spec->waveforms[i].waveform);
				if (!g_ascii_strncasecmp(mode, mode_name, strlen(mode_name))) {
		...
		case SR_CONF_PATTERN_MODE:
			...
			for (i = 0; i < ch_spec->num_waveforms; i++) {
				g_variant_builder_add(b, "s",
				 waveform_type_to_string(ch_spec->waveforms[i].waveform));
			}
			*data = g_variant_new("as", b);
			g_variant_builder_unref(b);
			break;
		case SR_CONF_OUTPUT_FREQUENCY:
			/* Frequency range depends on the currently active wave form */
			if (rigol_dg_get_channel_state(sdi, cg) != SR_OK)
				return SR_ERR_NA;
			wf_spec = get_waveform_spec(ch_spec, ch_status->wf);
			if (!wf_spec)
				return SR_ERR_BUG;
